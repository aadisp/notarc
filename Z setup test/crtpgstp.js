...
import { 
  getFirestore, doc, 
  updateDoc, deleteDoc, getDoc  } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
onAuthStateChanged(auth, async(user) => {
  ...
  document.title=`notarc - ${user.email} Dashboard`;
...
async function loadCart(user) {
  ...
  snapshot.forEach(docSnap => {
    ...
    total += item.price * item.quantity;
     ...
     titlecontainer.innerHTML=`
      <div class="total" id="cart-total">Total
      </div>

      <div class="checkout" onclick="buyAll()">
